...
class simpleSeblodAPI extends JCckContent{
	...
	public function loadContent( $id )
	{
		...
		c.storage_field
		FROM
		'.$app->getCfg('dbprefix').'cck_core_types AS a
		Left Join '.$app->getCfg('dbprefix').'cck_core_type_field As b ON b.typeid = a.id
		Left Join '.$app->getCfg('dbprefix').'cck_core_fields As c ON c.id = b.fieldid
		WHERE
		a.`name` = \''.$base->cck.'\' AND
		c.type = \'field_x\'
		');
